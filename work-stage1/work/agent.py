...
class AlphaBetaAgent:
    ...
    def get_max_value(self, game: Game, alpha: int, beta: int, depth: int):
        ...
        # possible_next_actions = self.prune(possible_next_actions)
    ...
    def prune(self, possible_actions: np.array) -> np.array:
        len_possible_actions = len(possible_actions)
        
        if len_possible_actions <= 10:
            return possible_actions

        extra = len_possible_actions - constants.MAX_POSSIBLE_ACTIONS

        extra_start = int(floor(extra / 2))
        extra_end = int(ceil(extra / 2))

        return possible_actions[extra_start: -extra_end]
