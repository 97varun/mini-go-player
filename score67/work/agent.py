...
class AlphaBetaAgent:
    ...
    def search(self, game: Game, last_move: int):
        self.player = game.curr_player

        _, action = self.get_max_value(game, self.neg_inf, self.pos_inf, 0, last_move)
    ...
    def get_max_value(self, game: Game, alpha: int, beta: int, depth: int, last_move: int):
        curr_state = game.get_board_state()
        ...
        # possible_next_actions = self.prune(possible_next_actions, last_move)
        ...
        for action in possible_next_actions:
            ...
            min_value = self.get_min_value(
                next_state, alpha, beta, depth + 1, action)[0]
    ...
    def get_min_value(self, game: Game, alpha: int, beta: int, depth: int, last_move: int):
        curr_state = game.get_board_state()
    ...
    def prune(self, possible_actions: np.array, last_move: int) -> np.array:
        len_possible_actions = len(possible_actions)

        if len_possible_actions <= constants.MAX_POSSIBLE_ACTIONS:
            np.random.shuffle(possible_actions)
            return possible_actions

        extra = len_possible_actions - constants.MAX_POSSIBLE_ACTIONS

        if last_move == -1:
            return np.random.choice(possible_actions, constants.MAX_POSSIBLE_ACTIONS)

        x, y = last_move // constants.BOARD_SIZE, last_move % constants.BOARD_SIZE

        sorted_possible_actions = list(possible_actions)
        sorted_possible_actions.sort(
            key=lambda a:
            self.manhattan_distance(x, y, a // constants.BOARD_SIZE, a % constants.BOARD_SIZE))

        return np.array(sorted_possible_actions[0: -extra])
